...
export default function ReceiptsManagement() {
  ...
  doc.setFontSize(20);
  doc.text("Receipt Details", 10, 15);

  doc.setFontSize(12);
  doc.text(`Receipt ID: ${receipt.id}`, 10, 30);
  doc.text(`Status: ${receipt.status}`, 10, 40);
  doc.text(`Supplier: ${receipt.supplier}`, 10, 50);
  doc.text(`Date: ${new Date(receipt.date).toLocaleDateString()}`, 10, 60);

  doc.text("Products:", 10, 80);

  let y = 90;
  receipt.products.forEach((p) => {
    doc.text(`• ${p.name} — ${p.qty} ${p.unit}`, 10, y);
    y += 10;
  });
...
//           className="flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-medium mb-4 group"
//         >
//           <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 transform group-hover:-translate-x-1 transition-all" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
...
//       <div className="space-y-4">
//         {filteredReceipts.map((receipt) => (
//           <div key={receipt.id} className="bg-white rounded-xl shadow-sm p-6">
...
//                 <div>
//                   <h3 className="text-lg font-bold">{receipt.id}</h3>
//                   <span className={`px-3 py-1 rounded-lg text-xs font-medium ${getStatusColor(receipt.status)}`}>
//                     {receipt.status}
//                   </span>
...
//                       <Building className="w-4 h-4" /> {receipt.supplier}
//                     </div>
//                     <div className="flex items-center gap-2">
//                       <Calendar className="w-4 h-4" /> {new Date(receipt.date).toLocaleDateString()}
//                     </div>
//                     <div className="flex items-center gap-2">
//                       <Package className="w-4 h-4" /> 
//                       {receipt.items} items • {receipt.totalQty} units total
//                     </div>
//                   </div>
//                 </div>
//               </div>
...
//                     setShowViewModal(true);
//                   }}
//                   className="px-4 py-2 text-blue-600 border rounded-lg"
//                 >
...
//                   <span className={`px-3 py-1 rounded-lg ${getStatusColor(selectedReceipt.status)}`}>
//                     {selectedReceipt.status}
//                   </span>
//                 </div>
//                 <div>
...
//                   <p className="font-semibold">{new Date(selectedReceipt.date).toLocaleDateString()}</p>
//                 </div>
//               </div>

//               <h3 className="font-semibold text-lg mt-6 mb-3">Products</h3>
...
//               <button
//                 onClick={() => downloadReceiptPDF(selectedReceipt)}
//                 className="w-full mt-6 px-4 py-3 bg-indigo-600 text-white rounded-xl"
//               >
//                 Download PDF
//               </button>
